...
fine_breakpoints<-function(breakpoints, acnt_valid_chr_afsmoothed)
{
  ...
  {
    interval_cnts <- acnt_valid_chr_afsmoothed[acnt_valid_chr_afsmoothed$V2>=as.numeric(breakpoints[bp, 2]) & acnt_valid_chr_afsmoothed$V2<=as.numeric(breakpoints[bp, 3]), ]
...
df_chrsizes <- read.table(chrsizefile, header = FALSE, sep = "\t")[,1:2]
...
pdf(paste(out_path, "/", barcode, "_allele_cnts_at_markers_sorted_co.pdf", sep=""),
    family="Helvetica", height=6, width=8.3)
par(mai = c(1.1, 1, 0.1, 1.2)); # margin: bottom, left, top, right
m <- cbind(1:chr_num)
...
if(sum(informativepos)>=min_marker_num){
  ...
  for (chr in 1:chr_num){
    ...
    plot(acnt_valid_chr[acnt_valid_chr$V4>0, 2], 
         ...
         ylim=c(-1*this_ylim,this_ylim), xlim=c(1, max(chrsize)),
         axes=F, xlab="Position", ylab="Allele count")
    axis(1, at=c(seq(0, chrsize[chr], 5e+07), chrsize[chr]),
         labels = round(c(seq(0, chrsize[chr], 5e+07), chrsize[chr])/1e+06), cex.axis=1.2)
    axis(2, at = c(-1*this_ylim, 0, this_ylim), labels = c(this_ylim, 0, this_ylim), cex.axis=1.2)
    ...
    #
    mtext(paste("(Mb chr", chr, ")\n"), at= chrsize[chr]+30e+06,  side=1, line=2.5, cex=1.2)
    #
    rect(1, this_ylim-0.5, chrsize[chr], this_ylim+2, col="gray", border = NA) # background
    #
    ...
    if(chr==min_chr_size_index){
      legend(chrsize[chr]+round(0.3*(chrsize[max_chr_size_index]-chrsize[chr])), 
             5,
             pch    = c(15, 15),
             col    = c("red", "blue"),
